module cross_coupled
  ! I have some general cross-coupled infrastructure in here
  ! you can see I got excited about the OOP paradigm for a bit
  ! while making this file. 
  use basic_IMSRG

  ! each structure has it's own annoying optimal storage for
  ! ph transformed matrix elements...  
  type :: ph_mat
  ...
  ! interface all of these things into one routine, so I don't have
  ! to keep track. 
  ! all of these functions do roughtly the same thing, so don't worry about
  ! their differences unless you are planning on doing something crazy.
...
!=======================================================
  subroutine allocate_CCMAT(OP,CCME,jbas) 
    ...
    ! the indexing strategy is designed to be fast! not intutive, SORRY.
    ...
    do q1 = 1, CCME%nblocks
       ...
       do i = 1, NX
          do j = 1,NX 
             ...
             x = CCindex(i,j,NX) 
             ! each (ij) pair maps to some integer by CCindex
             ...
             if ( (jbas%con(i) == 0 ).and. (jbas%con(j) == 1)) then
                nb = nb + 1
                CCME%nbmap(x)%Z(g) = nb
                ! This CCME%nbmap has the actual ket index
                ! in the current block. g depends on (J Pi TZ)
             end if
          ...
          do pg = 1, Ntot - Atot 
             ...
             do a = 1, HS%nsp
                do b = 1, HS%nsp
                   ...
                   ! figure out where the matrix element is
  ...
  subroutine calculate_single_pandya(OP,CCME,jbas,q1,term) 
    ...
    !                                                      |---<--| J2 
    ! this calculates the generalized panya matrix elements for 1 block (q1) 
    implicit none 
...
!!$omp end parallel do
  ...
  real(8) function WCCX(a,d,b,c,J1,J2,WX,RCC,jbas)
    ! these fucntions fetch matrix elements from
    ! the multiplied intermeidates for ph-ladder terms 
    implicit none
